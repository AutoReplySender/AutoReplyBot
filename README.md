...
# 每条语录是数组中的一个元素
  ...
  at_me: false # 可选，设置为true时仅bot（或其他指定用户名）被@到时才会触发，设置为false时仅bot不被@到时才会触发。默认无论是否@bot均可触发
  have_band_refer: true # 可选，设置为true时仅回复包含@时才会触发，设置为false时仅回复不包含@时才会触发。默认无论是否包含@均可触发
```
